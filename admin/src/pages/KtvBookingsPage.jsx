...
import { fetchKtvBookings, update } from '../api';
...
export default function KtvBookingsPage() {
  ...
  const load = useCallback(async () => {
    try {
      setError(null);
      const data = await fetchKtvBookings();
      setList(data || []);
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  return (
    <div>
      <h1 style={{ marginBottom: 16 }}>K歌房预约</h1>
      ...
      <div style={tableWrap}>
        <table style={table}>
          ...
          <tbody>
            {list.length === 0 ? (
              <tr>
                <td colSpan={7} style={{ ...td, textAlign: 'center', color: '#999' }}>暂无K歌房预约</td>
              </tr>
            ) : (
              list.map((row) => (
                <tr key={row.id}>
